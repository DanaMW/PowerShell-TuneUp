while (1) {
    ...
    [int]$newsize.height = ($BufHeight)
    [int]$newsize.width = ($BufWidth)
    ...
    if ($null -ne ($Net1.name)) {
        ...
        $N1btmp = (Convert-Size -From "Bytes" -To "GB" -Value $Net1.Speed)
        $Con14 = "$tt" + "Speed..$ESC[31m]$ESC[37m: $ESC[36m" + ($N1btmp) + "$ESC[37mGB"
