...
This is a tuned up second draft and is working pretty well on my machines with
PowerShell Core 6.0.X.

To use it place BinMenu.ps1 and BinMenu.json in the desired folder. Run it.
The settings in the json file are editable easily in the BMSM.ps1 Settings manager. BinMenu
will auto create the needed INI on run or there is a menu option to run it. Just say yes or no to each.
...
Update: Added new $SortMethod for scripts can sort 0-Alphabetical 1-Random 2-Length of scriptname.
    ...
    AddItems can now also be a Directory and explorer will open them.
...
Update Delete 115 now works. Deletes the last entry for now. Going to make it programable this evening.
```
